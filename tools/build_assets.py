...
def emit_ds_asset_header(scenes, sprites, palettes) -> None:
    ...
    w("#include <cstdint>")
    w("")
    w("// vram_map.h checks its object-VRAM reservation against OBJ_REACH and")
    w("// OBJ_RESIDENT_BYTES below, but only when this header has been included --")
    w("// it has to stay standalone, because the device tier includes it before")
    w("// anything else exists.  This is how it knows.")
    w("#define KH_ASSETS_H_INCLUDED 1")
    w("")
    w("namespace kh {")
    w("")
    ...
    w("// DERIVED, not emitted as a literal.  A literal here can disagree with the")
    w("// boundary above, and that disagreement is invisible: every tile number is")
    w("// still in range, the budget check still passes, and the sprites are simply")
    w("// at the wrong addresses.  Deriving it means changing the boundary moves the")
    w("// reach with it, and vram_map.h's reservation check sees the new value.")
    w("constexpr int OBJ_REACH = (MAP_TILE_MASK + 1) * OBJ_BOUNDARY;")
    w("static_assert(OBJ_REACH == %d, \"regenerate: the boundary changed\");"
      % DS_OBJ_REACH)
